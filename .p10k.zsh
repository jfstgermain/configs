# Generated by Powerlevel10k configuration wizard on 2023-02-22 at 14:32 EST.
...
() {
  ...
  function my_git_formatter() {
	...
	if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
	  ...
	  # Extract the matched part according to the given regex
      local matched_part=$(echo "$branch" | awk 'match($0, /(feature|bugfix)\/[[:alnum:]]+-[[:digit:]]+/) {print substr($0, RSTART, RLENGTH)}')

      #local matched_part=$(echo "$branch" | grep -o '(feature|bugfix)\/\w+-\d+')

      # Find the length of the matched part
      local length=${#matched_part}

      # Replace the previous hard-coded value 17 with the calculated length
      (( $#branch > 32 )) && branch[$length+1,-13]="…"
    ...
    #  (( $#branch > 32 )) && branch[17,-13]="…"  # <-- this line
    #  res+="${clean}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}"
    #fi
